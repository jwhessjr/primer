...
offeredActionTest sh l inputExpr position action expectedOutput = do
  ...
  action' <- case action of
    Left a -> do
      ...
      pure $ toProgActionNoInput (foldMap' moduleDefsQualified $ progModules prog) exprDef (Selection exprDefName $ Just $ NodeSelection BodyNode id) a
    Right (a, o) -> do
      ...
      case options a of
        ...
        Just os -> do
          ...
          pure $ toProgActionInput exprDef (Selection exprDefName $ Just $ NodeSelection BodyNode id) o a
  action'' <- case action' of
    Left err -> assertFailure $ show err
    Right a -> pure a
